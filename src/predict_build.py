...
if __name__=='__main__':
    ...
    prediction_input = pd.read_csv(config.TRAINING_FILE)
    prediction_input=prediction_input[prediction_input['kfold']==0].reset_index(drop=True)
    df=prediction_input
    #df['kfold']=0
    ...
    predictions['Prediction Bucket']=predictions['Decile'].apply(lambda x: "1.High Risk" if (x =='1') else ("2.Medium Risk" if (x=='2') else ("3.Low Risk")))

    predictions=predictions.drop(['kfold'],axis=1)

    


    predictions=predictions[['Emp.No.','Prediction Probability','Decile','Prediction Bucket','Attrition']]
    predictions['Intervention']="Not Recommended"
    predictions.loc[predictions['Prediction Bucket']=="1.High Risk",'Intervention']="Strictly Recommended"
    predictions.loc[predictions['Prediction Bucket']=="2.Medium Risk",'Intervention']=predictions['Prediction Probability'].rank(ascending=False,method='first')
    
    predictions=predictions.sort_values(by="Prediction Probability",ascending=False).reset_index(drop=True)
    
    
    predictions['Percentile']=pd.qcut(predictions['Prediction Probability'],[0,0.85,1],labels=["low","high"])
    print(predictions)
    print(predictions['Percentile'].value_counts())
    ...
    # total_pred=predictions[predictions['Prediction Bucket']=="1.High Risk"]
    # actual_res=oot[(oot['Prediction Bucket']=="1.High Risk")|(oot['Prediction Bucket']=="2.Medium Risk")|(oot['Prediction Bucket']=="3.Low Risk")]
    # correct_pred=oot[(oot['Prediction Bucket']=="1.High Risk")]
    ...
    df_ks=predictions
    df_ks.rename(columns={'Attrition':'status'},inplace=True)
    ...
    df_ks.to_excel("../output/ks_table_build.xlsx")
